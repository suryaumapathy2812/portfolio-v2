...
async function createMdFiles() {
  ...
  for (let page of pages) {
    ...
    const { title: _title, description: _description, slug: _slug, created_at: _created_at } = page.properties

    const title = _title.title[0].plain_text;
    const desc = _description.rich_text.reduce((acc, cur) => acc + cur.plain_text, "")
